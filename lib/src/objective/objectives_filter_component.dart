...
/*
    int index;
    _objectiveService.objectiveFilterOrder.groupIds.forEach((item) {
      index = groupOptions.optionsList.indexWhere((t) => t.id == item);
      if (index != -1) groupMultiSelectModel.select(groupOptions.optionsList[index]);
    });

    showPopupChangeGroup(false);

    _objectiveService.objectiveFilterOrder.leaderUserIds.forEach((item) {
      index = leaderOptions.optionsList.indexWhere((t) => t.id == item);
      if (index != -1) leaderMultiSelectModel.select(leaderOptions.optionsList[index]);
    });

    showPopupChangeLeader(false);

    archived = _objectiveService.objectiveFilterOrder.archived;
*/
  }
  ...
  void applyFilter() async {
    ...
    _objectiveService.objectiveFilterOrder.filteredItems = 0;

    _objectiveService.objectiveFilterOrder.groupIds = groupMultiSelectModel.selectedValues.map((f) => f.id).toSet().cast();
    _objectiveService.objectiveFilterOrder.filteredItems += _objectiveService.objectiveFilterOrder.groupIds.length;
    _objectiveService.objectiveFilterOrder.leaderUserIds = leaderMultiSelectModel.selectedValues.map((f) => f.id).toSet().cast();
    _objectiveService.objectiveFilterOrder.filteredItems += _objectiveService.objectiveFilterOrder.leaderUserIds.length;
    _objectiveService.objectiveFilterOrder.archived = archived;
    _objectiveService.objectiveFilterOrder.filteredItems += (archived) ? 1 : 0;
    _objectiveService.objectiveFilterOrder.orderedBy = orderedBy;
  ...
  void showPopupChangeGroup(bool event) {
    ...
    if (selectedValues != null && selectedValues.isNotEmpty) {
      if (selectedValues.length == 1) {
        ...
        groupInputText =  "${groupItemRenderer(selectedValues.first)} + ${selectedValues.length - 1} ${moreLabel}";
      }
    }
  }
  ...
  void showPopupChangeLeader(bool event) {
    ...
    if (selectedValues != null && selectedValues.isNotEmpty) {
      if (selectedValues.length == 1) {
        ...
        leaderInputText =  "${leaderItemRenderer(selectedValues.first)} + ${selectedValues.length - 1} ${moreLabel}";
      }
    }
  }
 ...
 /*   var it = groupMultiSelectModel.selectedValues.iterator;
    while (it.moveNext()) {
      groupMultiSelectModel.deselect(it.current);
    }

    print('DEBUG CLEAR ${groupMultiSelectModel.selectedValues.length}');
*/
  ...
  void clearLeader() {
/*
    var it = leaderMultiSelectModel.selectedValues.iterator;
    while (it.moveNext()) {
      leaderMultiSelectModel.deselect(it.current);
    }
*/
...
/*
  String get superGroupLabelRenderer {
    String nameLabel;
    if ((groupMultiSelectModel != null &&
        groupMultiSelectModel.selectedValues != null &&
        groupMultiSelectModel.selectedValues.length != null)) {

      nameLabel = groupMultiSelectModel.selectedValues.first.name;
    }
...
*/
 ...
 // FactoryRenderer get labelFactory => (_) => objectives_filter_component.ExampleLabelRendererComponentNgFactory;
